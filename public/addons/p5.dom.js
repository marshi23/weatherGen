...
(function(root, factory) {
  ...
  p5.prototype.selectAll = function(e, p) {
    ...
    if (res) {
      for (let j = 0; j < res.length; j++) {
        let obj = this._wrapElement(res[j]);
        arr.push(obj);
      }
    }
    return arr;
  ...
  p5.prototype._wrapElement = function(elt) {
    let children = Array.prototype.slice.call(elt.children);
    if (elt.tagName === 'INPUT' && elt.type === 'checkbox') {
      let converted = new p5.Element(elt);
  ...
  function addElement(elt, pInst, media) {
    let node = pInst._userNode ? pInst._userNode : document.body;
    node.appendChild(elt);
    let c = media ? new p5.MediaElement(elt) : new p5.Element(elt);
  ...
  tags.forEach(function(tag) {
    let method = 'create' + tag.charAt(0).toUpperCase() + tag.slice(1);
  ...
  p5.prototype.createCheckbox = function() {
    ...
    let self = addElement(elt, this);
    self.checked = function() {
      let cb = self.elt.getElementsByTagName('input')[0];
      if (cb) {
        if (arguments.length === 0) {
          return cb.checked;
        } else if (arguments[0]) {
  ...
  p5.prototype.createRadio = function(existing_radios) {
    ...
    self.selected = function(value) {
      let i;
      let inputChildren = self._getInputChildrenArray();
      if (value) {
  ...
  p5.prototype.createFileInput = function(callback, multiple) {
    ...
    function handleFileSelect(evt) {
      function makeLoader(theFile) {
        ...
        let p5file = new p5.File(theFile);
        return function(e) {
  ...
  function createMedia(pInst, type, src, callback) {
    ...
    let c = addElement(elt, pInst, true);
  ...
  p5.prototype.createCapture = function() {
    ...
    let c = addElement(elt, this, true);
    c.loadedmetadata = false;
    // set width and height onload metadata
    elt.addEventListener('loadedmetadata', function() {
      elt.play();
  ...
  p5.Element.prototype.center = function(align) {
    let style = this.elt.style.display;
    let hidden = this.elt.style.display === 'none';
    let parentHidden = this.parent().style.display === 'none';
    let pos = { x: this.elt.offsetLeft, y: this.elt.offsetTop };
    ...
    let wOffset = Math.abs(this.parent().offsetWidth - this.elt.offsetWidth);
    let hOffset = Math.abs(this.parent().offsetHeight - this.elt.offsetHeight);
    let y = pos.y;
    let x = pos.x;
  ...
  p5.Element.prototype.style = function(prop, val) {
    ...
    if (typeof val === 'undefined') {
      ...
      if (prop === 'rotate' || prop === 'translate' || prop === 'position') {
        let trans = Array.prototype.shift.apply(arguments);
        let f = this[trans] || this['_' + trans];
  ...
  p5.MediaElement = function(elt, pInst) {
    ...
    Object.defineProperty(self, 'src', {
      get: function() {
        let firstChildSrc = self.elt.children[0].src;
        let srcVal = self.elt.src === window.location.href ? '' : self.elt.src;
        let ret =
  ...
  p5.MediaElement.prototype.loadPixels = function() {
    ...
    if (this.loadedmetadata) {
      ...
      let currentTime = this.elt.currentTime;
  ...
  p5.MediaElement.prototype._onTimeUpdate = function() {
    let playbackTime = this.time();

    for (let i = 0; i < this._cues.length; i++) {
      let callbackTime = this._cues[i].time;
      let val = this._cues[i].val;
